...
function showPopup(message, type = "success") {
  ...
  // set className correctly as a string
  ...
  // style the border according to type
  popup.style.border = type === "error"
    ? "2px solid rgba(255,50,50,0.8)"
    : "2px solid rgba(50,255,100,0.8)";

  // use a template literal for innerHTML
  popup.innerHTML = `
    <div class="text-2xl mb-2">${type === "error" ? "❌" : "✅"}</div>
    <div class="text-sm">${message}</div>
  `;

  // append and auto-remove after a delay
  document.body.appendChild(popup);
  setTimeout(() => {
    popup.classList.add('popup-fade'); // optional, if you want fade CSS
    setTimeout(() => popup.remove(), 400); // remove after fade
  }, 2000);
}
