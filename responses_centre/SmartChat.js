...
// Premium model toggle logic (must be inside bindEvents so it works after dynamic HTML)
this.premiumModels = [
...
window.state.premiumModel = window.state.premiumModel || 'gpt-4.1-nano';

const premiumBtn = document.getElementById('premiumChatBtn');
const updatePremiumBtn = () => {
  if (!premiumBtn) return;
  const model = this.premiumModels[this.premiumIdx];
  premiumBtn.classList.remove('nano', 'mini', 'full');
  premiumBtn.classList.add(model.key);
  premiumBtn.querySelector('.premium-label').textContent = model.label;
  premiumBtn.querySelector('.premium-desc').textContent = model.desc;
};
if (premiumBtn) {
  premiumBtn.onclick = () => {
    this.premiumIdx = (this.premiumIdx + 1) % this.premiumModels.length;
    updatePremiumBtn();
    showToast(this.premiumModels[this.premiumIdx].toast, 'info');
    window.state.premiumModel =
      this.premiumModels[this.premiumIdx].key === 'nano'
        ? 'gpt-4.1-nano'
        : this.premiumModels[this.premiumIdx].key === 'mini'
        ? 'gpt-4.1-mini'
        : 'gpt-4.1-full';
  };
  updatePremiumBtn();
...
async handleFileUpload(e) {
  ...
  for (const file of files) {
    ...
    if (isImage || isPdf || isDocx) {
      await this.handleFileOrUrlAttachment('file', file);
    } else {
