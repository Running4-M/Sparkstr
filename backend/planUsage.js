...
export async function checkAndUpdateUsage(feature) {
  ...
  const usageByDate = settings.usageByDate || {};
  ...
  // Check limit (prefer usageByDate for today)
  const used = usageByDate[today]?.[feature] ?? usage[feature] ?? 0;
  if (used >= (limits[feature] || 0)) {
    return false; // Over limit
  }

  // Increment and save both
  usage[feature] = (usage[feature] || 0) + 1;
  usageByDate[today] = usageByDate[today] || {};
  usageByDate[today][feature] = (usageByDate[today][feature] || 0) + 1;
  usageByDate[today].lastUsageReset = today;
  await saveUserSettings({ ...settings, usage, usageByDate });
  return true;
}
