...
function renderWeekView() {
  ...
  html += `
    <!-- Time column -->
    <div 
     class="h-16 border-b border-r border-border text-xs text-muted-foreground flex items-start justify-end pr-2 pt-1 sticky left-0 bg-background/80 backdrop-blur-sm z-30"
    >
  ...
  return `
    <div class="space-y-4">
      ...
      <div>
        ...
        <div>
  <label class="block text-sm font-medium mb-1" for="description">Description</label>
  <div class="markdown-description" style="font-size:1em; background:rgba(51,65,85,0.13); border-radius:8px; padding:1rem 1.5rem;">
    ${window.md.render(event.description || '')}
  </div>
</div>
      </div>
...
async function processAIResponses() {
  ...
  try {
    ...
    // Filter events: AI-enabled OR isComplex, and no response yet
    const aiEvents = [];
    for (const event of todaysEvents) {
      // If event is complex OR aiEnabled, and no response exists
      if ((event.aiEnabled || event.isComplex) && !(await checkExistingResponse(event.id))) {
        aiEvents.push(event);
      }
    }

    if (aiEvents.length === 0) {
      console.log('No AI-enabled or complex events to process.');
      return;
    }
    ...
    for (const event of aiEvents) {
      try {
        ...
        if (!response.ok) {
          throw new Error(`AI response failed (${response.status}): ${await response.text()}`);
        }
        ...
        showToast('AI Response', `AI response generated for "${event.title}"`, 'success');
      } catch (err) {
        console.error('Error processing AI event:', event, err);
        showToast('AI Error', `Failed for "${event.title}": ${err.message}`, 'error');
      }
    }
  } catch (error) {
