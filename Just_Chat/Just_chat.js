...
// current currency (auto-detected, can be changed by dropdown)
let currentCurrency = detectCurrency();

// This will hold a reference to the plan-render function (if you want re-rendering)
let renderCardsFunction = null;

// initialize the currency selector dropdown in a modal DOM subtree (containerRoot)
function initCurrencySelector(containerRoot = document) {
  const select = containerRoot.querySelector("#currency-selector-select");
  if (!select) return;
  // ensure currentCurrency exists in PRICE_MAP.basic (fallback to USD)
  ...
  select.addEventListener("change", (e) => {
    ...
    if (PRICE_MAP.basic[newCurrency]) {
      ...
      // If plan chooser is visible, re-render cards
      if (renderCardsFunction) renderCardsFunction();
      // Also update any inline plan price labels
...
// Add after imports
// Update the changePlanFlow function
// --- Replace the existing changePlanFlow with this hardened version ---
async function changePlanFlow(planKey, settings = null) {
  ...
  // Immediate free downgrade (no Stripe)
  if (planKey === "free") {
    try {
      // Attempt to mark subscriptions to cancel (best effort client-side read)
      try {
        const subsCol = collection(db, "customers", uid, "subscriptions");
        const subs = await getDocs(subsCol);
        if (!subs.empty) {
          const batch = writeBatch(db);
          subs.forEach(snap => {
            // client cannot modify extension-written subs safely — we avoid heavy writes.
            // If you want to set cancel_at_period_end, do it server-side or via Stripe.
            // Here we just keep user's plan in Firestore as free.
          });
          // No batch.commit() needed if no writes were added above.
        }
      } catch (innerErr) {
        // ignore errors reading subscriptions
        console.warn("Could not read subscriptions while downgrading:", innerErr);
      }
      ...
      showPopup("Successfully downgraded to Free plan", "success");
  ...
  try {
    ...
    const currentSection = (window.sidebar && window.sidebar.getActiveSection && window.sidebar.getActiveSection()) || '';

    // record previous plan and mark pending (so UI doesn't treat user as paid)
    const userSnapshot = await getDoc(userRef);
    const prevPlan = (userSnapshot && userSnapshot.exists() && userSnapshot.data().plan) ? userSnapshot.data().plan : "free";
    ...
    // Create checkout_sessions doc for Firebase Stripe Extension
    const priceId = getPriceId(planKey);
if (!priceId) {
  throw new Error("Price ID not configured for plan: " + planKey + " / currency: " + currentCurrency);
}

// persist currency metadata in the checkout session so the extension / backend knows what user expected
    ...
    // Keep the existing onSnapshot redirect listener you had (unchanged)
    ...
    // at this point browser should be redirected by the extension; function returns
  } catch (error) {
    console.error("Plan change error:", error);
    showPopup("Failed to change plan: " + (error.message || error), "error");

    // revert immediate changes if possible
    try {
      await updateDoc(userRef, {
        plan: (settings && settings.plan) ? settings.plan : "free",
        requestedPlan: null,
        pendingAt: null,
        prevPlan: null
      }).catch(()=>{});
    } catch (e) { /* ignore revert errors */ }
  }
}
  ...
  const unsub = onSnapshot(subsCol, (snapshot) => {
    ...
    snapshot.forEach(snap => {
      ...
      if (data.status === 'active' || data.status === 'trialing') {
        ...
        nextBillingEl.textContent = `Next billing date: ${date.toLocaleDateString()}`;
        return;
      }
    });
...
updatePlanPrices(modalContent);
  ...
  if (planCardContainer) {
    ...
    // Add click handler for change plan button
    const changePlanBtn = planCardContainer.querySelector('#changePlanBtn');
    if (changePlanBtn) {
    ...
    // Add next billing text element after plan container
    const nextBillingTextEl = document.createElement('p');
    nextBillingTextEl.id = 'nextBillingText';
    nextBillingTextEl.style.cssText = 'color:#9fb0db;font-size:13px;margin-top:8px;';
    planCardContainer.after(nextBillingTextEl);

    // Load billing info
    cleanupStalePending(auth?.currentUser?.uid).then(() => {
  loadBillingInfo();
});
  }
