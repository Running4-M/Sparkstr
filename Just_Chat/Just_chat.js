...
async function changePlanFlow(planKey, settings = null) {
  if (!auth.currentUser) {
    showToast("Please sign in to change your plan.", "error");
    return;
  }
  const uid = auth.currentUser.uid;

  // Handle free downgrades immediately
  if (planKey === 'free') {
    try {
      ...
      subsSnapshot.forEach(docSnap => {
        if (docSnap.data().status === 'active') {
          batch.update(docSnap.ref, { cancel_at_period_end: true });
        }
      });
  ...
  try {
    ...
    if (currentSubscriptionId) {
      try {
        ...
        const amountDue = preview.invoicePreview.amount_due ?? 0;
const humanAmount = preview.invoicePreview.human_amount_due || (amountDue / 100).toFixed(2);
...
showProrationModal({
  ...
  onConfirm: async () => {
    try {
      await performProratedUpgrade(currentSubscriptionId, newPriceId);

      // Close settings modal
      window.dispatchEvent(new CustomEvent('closeSettingsModal'));

      if (isCredit) {
        showToast(`Successfully downgraded to ${planKey}. A credit of ${humanAmount} was applied.`);
      } else {
  ...
  onCancel: async () => {
    await updateDoc(doc(db, "users", uid), {
      ...
      requestedPlan: null
        ...
        showToast("Prorated upgrade failed: " + (err.message || err), "error");
      }
    }
  } catch (err) {
    console.error("Plan change error:", err);
    showToast("Plan change failed: " + (err.message || err), "error");
  }
}
...
function showProrationModal({ planKey, humanAmount, isCredit, onConfirm, onCancel }) {
  ...
  const title = isCredit
    ? `Downgrade to ${planKey.charAt(0).toUpperCase() + planKey.slice(1)}`
    : `Upgrade to ${planKey.charAt(0).toUpperCase() + planKey.slice(1)}`;

  const message = isCredit
    ? `You will receive a <strong>${humanAmount}</strong> credit. This will be applied to your next invoice.`
    : `You will be charged <strong>${humanAmount}</strong> now (prorated).`;
  ...
  overlay.innerHTML = `
    <div style="
      ...
      text-align:center;
    ">
      <h2 style="color:#6366f1;font-size:1.4rem;margin-bottom:1rem;">
        ${title}
      </h2>
      <p style="font-size:1.05rem;color:#374151;margin-bottom:1.5rem;">
        ${message}
      </p>
      <div style="display:flex;gap:1rem;justify-content:center;">
        ...
        <button id="confirmUpgradeBtn" style="background:#6366f1;color:#fff;padding:0.6em 1.2em;border:none;border-radius:8px;font-weight:600;cursor:pointer;">
          Confirm
        </button>
      </div>
    </div>
...
async function fetchBillingInfoOnce() {
  ...
  if (activeSub.creditBalance && activeSub.creditBalance > 0) {
    const creditEl = document.createElement("p");
    creditEl.style = "color:#059669;font-weight:500;margin-top:8px;";
    creditEl.textContent = `You have £${(activeSub.creditBalance/100).toFixed(2)} credit.`;
    planCardContainer.appendChild(creditEl);
  }
  ...
  creditBalance: activeSub?.creditBalance || 0
};
