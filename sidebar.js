...
showProrationModal({
  ...
  onConfirm: async () => {
    try {
      ...
      if (isCredit) {
        showPopup(`Successfully downgraded to ${planKey}. A credit of ${humanAmount} was applied.`);
      } else {
        showPopup(`Successfully upgraded to ${planKey}. You were charged ${humanAmount} (prorated).`);
      }

      window.location.href = window.location.origin + "/Calendar/Calendar";
    } catch (err) {
      console.error("Upgrade/downgrade failed", err);
      showPopup("Plan update failed: " + (err.message || err), "error");
    }
  },
  onCancel: async () => {
    await updateDoc(doc(db, "users", uid), {
