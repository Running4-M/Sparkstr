...
import { messaging} from './backend/firebase.js';
        ...
        <div style="display: flex; flex-direction: column; gap: 16px;">
          ...
          <div style="display:flex;flex-direction:column;gap:12px;">
            <div style="display:flex;align-items:center;justify-content:space-between;">
              ...
              <label id="labelEmailNotif" class="toggle-label" style="position: relative; display: inline-flex; align-items: center; cursor: pointer;">
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;">
              ...
              <label id="labelSMSNotif" class="toggle-label" style="position: relative; display: inline-flex; align-items: center; cursor: pointer;">
...
if (confirmDeleteBtn) {
  ...
  function wireToggle(labelId, { onBg = 'rgb(37 99 235)', offBg = 'rgb(100 116 139)', onChange } = {}) {
    ...
    const setVisual = (checked) => {
      ...
      // set left/right so animation and placement are independent of widths
      if (checked) {
        dot.style.left = 'auto';
        dot.style.right = '2px';
      } else {
  ...
  const emailToggle = wireToggle('labelEmailNotif');
  ...
  const smsToggle = wireToggle('labelSMSNotif');
  ...
  initCurrencySelector(modalContent);
    ...
    if (emailToggle && emailToggle.checkbox) {
      emailToggle.checkbox.checked = !!s.emailNotifications;
      emailToggle.setVisual && emailToggle.setVisual(emailToggle.checkbox.checked);
    }
    if (pushToggle && pushToggle.checkbox) {
      pushToggle.checkbox.checked = !!s.pushNotifications;
      pushToggle.setVisual && pushToggle.setVisual(pushToggle.checkbox.checked);
    }
    if (smsToggle && smsToggle.checkbox) {
      smsToggle.checkbox.checked = !!s.smsNotifications;
      smsToggle.setVisual && smsToggle.setVisual(smsToggle.checkbox.checked);
    }
    

    // render plan card (keeps your existing logic)
    ...
    planCardContainer.innerHTML = `
      <div class="pm-plan-card ${planClass}">
        ...
        <div class="pm-plan-price">${planKey !== 'free' ? PRICE_MAP[planKey]?.[currentCurrency]?.amount || 'Price not available' : 'Free'}</div>
      </div>
    `;
