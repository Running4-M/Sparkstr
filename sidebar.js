  ...
  try {
    ...
    if (currentSubscriptionId) {
      try {
        ...
        const humanAmount = preview.invoicePreview.human_amount_due ||
          (preview.invoicePreview.amount_due != null ? (preview.invoicePreview.amount_due / 100).toFixed(2) : null);
        ...
        showProrationModal({
          ...
          onConfirm: async () => {
            try {
              ...
              showPopup("Failed to upgrade: " + (err.message || err), "error");
            }
          },
          onCancel: async () => {
            await updateDoc(doc(db, "users", uid), {
...
function showProrationModal({ planKey, humanAmount, onConfirm, onCancel }) {
  ...
  overlay.innerHTML = `
    ...
    ">
      ...
      <p style="font-size:1.05rem;color:#374151;margin-bottom:1.5rem;">
        You will be charged <strong>${humanAmount}</strong> now (prorated).<br/>
        By clicking confirm, you agree to be charged immediately and your subscription will be upgraded.
      </p>
