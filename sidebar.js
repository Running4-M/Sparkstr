...
async function startCheckoutWithExtension(priceId) {
  ...
  try {
    const docRef = await addDoc(collection(db, "customers", uid, "checkout_sessions"), {
      ...
      cancel_url: window.location.origin + "https://sparkstr.com/Calendar/Calendar",
      mode: "subscription",
...
button.addEventListener('click', () => {
  ...
  if (window.innerWidth < 768) {
    ...
    createSidebarElement._feedbackFormOpen = true;
    render();
  } else {
    // On desktop, toggle inline feedback
...
const usageHtml = keys.map(k => {
  // Prefer usageByDate for today, fallback to flat usage object
...
<div id="deleteConfirmModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.75);z-index:100;align-items:center;justify-content:center;">
  ...
  // Add loading overlay (before modalContent is appended to modal)
...
loadingOverlay.innerHTML = `
  <div style="color:#fff;font-size:18px;font-weight:600;display:flex;flex-direction:column;align-items:center;gap:18px;">
    <span style="display:inline-block;width:38px;height:38px;border:4px solid #64748b;border-top-color:#2563eb;border-radius:50%;animation:spin 1s linear infinite;"></span>
    Loading...
  </div>
  <style>
...
if (confirmDeleteBtn) {
  ...
  renderPlanCard(modalContent, s);
     // Now fetch billing info, then hide loader overlay
...
if (expanded === planKey && !showLimits[planKey] && planKey !== settings.plan) {
  ...
  try {
    const currentSubscriptionId = await getActiveSubscriptionId(uid);
    // If user has no active subscription in Stripe, just update Firestore
    if (!currentSubscriptionId) {
      const batch = writeBatch(db);
      batch.update(doc(db, "users", uid), { plan: "free", requestedPlan: null });
      batch.set(doc(db, "users", uid, "settings", "profile"), {
    ...
    // Preview invoice with newPriceId = null so we can calculate refund (if any)
    const preview = await previewProration(currentSubscriptionId, null);
    if (!preview || !preview.invoicePreview) {
      throw new Error('No preview received for cancellation');
    }

    const netCents = preview.invoicePreview?.total_with_credit ?? preview.invoicePreview?.amount_due ?? 0;
    const refundAmountCents = netCents < 0 ? Math.abs(netCents) : 0;
